...
{
  FILE* file = 0;
  std::string strPath(program_name);
  std::string strFile;
  if (vertical)
      strFile = strPath + "/serf/geo/vertcs.csv";
  else
  {
      if(pcs)
          strFile = strPath + "/serf/geo/pcs.csv";
      else
          strFile = strPath + "/serf/geo/gcs.csv";
  }
...
bool GeoProjectionConverter::set_VerticalCSTypeGeoKey(short value, char* description)
{
  ...
  else
  {
    ...
    if (file == 0)
    {
      fprintf(stderr, "ERROR: cannot open 'vertcs.csv' file. maybe your XD LASToLAS distribution\n");
      fprintf(stderr, "       has no .\\LASToLAS\\bin\\serf\\geo\\vertcs.csv file. download the\n");
...
bool GeoProjectionConverter::set_gcs(short code, char* description)
{
  ...
  else
  {
    ...
    if (file == 0)
    {
      fprintf(stderr, "ERROR: cannot open 'gcs.csv' file. maybe your XD LASToLAS distribution\n");
      fprintf(stderr, "       has no .\\LASToLAS\\bin\\serf\\geo\\gcs.csv file. download the\n");
      //fprintf(stderr, "       latest version at http://lastools.org/download/LAStools.zip\n");
      return false;
    }
    int value = 0;
...
bool GeoProjectionConverter::set_epsg_code(short value, char* description, bool source)
{
  ...
  default:
    ...
    {
      fprintf(stderr, "ERROR: %s\n", argv_zero);
      fprintf(stderr, "ERROR: cannot open 'pcs.csv' file. maybe your XD LASToLAS distribution\n");
      fprintf(stderr, "       has no .\\LASToLAS\\bin\\serf\\geo\\pcs.csv file. download the\n");
      //fprintf(stderr, "       latest version at http://lastools.org/download/LAStools.zip\n");
      return false;
    }
    int epsg_code = 0;
